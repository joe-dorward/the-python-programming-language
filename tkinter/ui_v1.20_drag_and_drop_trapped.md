...
|||
|-|-|
|![ui_v1.20_drag_and_drop_trapped.py](illustrations/ui_v1.20_drag_and_drop_trapped_first.png)|![ui_v1.20_drag_and_drop_trapped.py](illustrations/ui_v1.20_drag_and_drop_trapped_second.png)|

```Python
# ---------- ---------- ---------- ---------- ---------- ---------- ---------- ----------
# Program ui_v1.20_drag_and_drop_trapped.py
...
# * adds the menubar_1
# ui_v1.20_drag_and_drop_trapped.py
# * modifies the <B1-Motion> event handler (method) by setting boundaries to trap the 
#   draggable object (the blue 'Label' widget)
...
def draggable_Object_Motion(event):
    ...
    # trap it
    if (mouse_x < left):
        draggable_object.place(x=left - 1, anchor="center")

    if (mouse_x > right):
        draggable_object.place(x=right - 1, anchor="center")

    if (mouse_y < top):
        draggable_object.place(y=top - 1, anchor="center")

    if (mouse_y > bottom):
        draggable_object.place(y=bottom - 1, anchor="center")
# ---------- ---------- ---------- ---------- ---------- ---------- ---------- ----------
