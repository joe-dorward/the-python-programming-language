...
This version, adds a `Label` an `Entry` widgets, and a `StringVar`. Know that the `entry_one` widget **_MUST BE_** associated with the `entry_one_string_variable` by its `textvariable` value.
